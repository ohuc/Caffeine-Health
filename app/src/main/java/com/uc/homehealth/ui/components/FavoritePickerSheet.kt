...
private fun iconKeyForDomain(domain: String): String = when (domain) {
    "light" -> "bulb"
    "switch", "input_boolean" -> "bulb"
    "climate" -> "thermo"
    "lock" -> "lock"
...
fun FavoritePickerSheet(
    ...
    val pool = remember(allEntities, existingIds) { allEntities.filter { it.entityId !in existingIds } }
    ...
    Tap(onClick = onPick, modifier = modifier) {
        Row(
            ...
            Box(
                ...
                Icon(
                    imageVector = haIconFor(iconKeyForDomain(entity.domain)),
                    contentDescription = null,
                    tint = cs.onSurface,
