...
fun ClimateEntityPickerSheet(
    ...
    val pool = remember(allEntities, existingIds) {
        allEntities.filter { it.domain == "climate" && it.entityId !in existingIds }
    }
    ...
    AppBottomSheet(visible = visible, onDismiss = onDismiss, hazeState = hazeState, maxHeightFraction = 0.92f) {
        Column(
            ...
            Text(
                text = "Choose a Home Assistant climate entity",
        ...
        if (pool.isEmpty()) {
            Box(
                ...
                Text(
                    text = "No climate entities found.",
                    fontFamily = MontserratFamily,
                    fontSize = 13.sp,
                    color = cs.onSurfaceVariant,
                )
