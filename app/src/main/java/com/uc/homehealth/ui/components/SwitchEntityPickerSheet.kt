...
fun SwitchEntityPickerSheet(
    ...
    val pool = remember(allEntities, existingIds) {
        allEntities.filter { it.domain == "switch" && it.entityId !in existingIds }
    }
    ...
    AppBottomSheet(visible = visible, onDismiss = onDismiss, hazeState = hazeState, maxHeightFraction = 0.92f) {
        Column(
            ...
            Text(
                text = "Choose a Home Assistant switch entity",
