...
fun LocationEntityPickerSheet(
    ...
    val pool = remember(allEntities, existingIds) {
        allEntities
            .filter { it.domain == "person" && it.entityId !in existingIds }
            // Entities currently reporting coordinates first — they're the ones that
            // can actually draw a map; presence-only people fall to the bottom.
            .sortedWith(compareByDescending<HaEntitySummary> { it.hasLocation }.thenBy { it.friendlyName.lowercase() })
    }
    ...
    AppBottomSheet(visible = visible, onDismiss = onDismiss, hazeState = hazeState, maxHeightFraction = 0.92f) {
        Column(
            ...
            Text(
                text = "Choose a Home Assistant person entity to map",
...
// Green GPS pill for entities reporting coordinates; muted label for presence-only
// people (router/ping trackers) that can't draw a map.
@Composable
private fun LocationTag(hasLocation: Boolean) {
    val cs = MaterialTheme.colorScheme
    if (hasLocation) {
        Row(
            verticalAlignment = Alignment.CenterVertically,
...
private fun PersonEntityRow(
    ...
    Tap(onClick = onPick, modifier = modifier) {
        Row(
            ...
            Box(
                modifier = Modifier.size(40.dp).background(cs.surfaceVariant, CircleShape),
                contentAlignment = Alignment.Center,
            ) {
                Icon(
            ...
            LocationTag(hasLocation = entity.hasLocation)
        }
    }
}
